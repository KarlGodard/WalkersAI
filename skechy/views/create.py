...
@skechy.app.route("/accounts/create/", methods=["POST", "GET"])
def show_create():
...
@skechy.app.route("/accounts/", methods=["POST"])
    ...
    if flask.request.form["operation"] == "update_password":
        ...
        for file in files:
            file_path = os.path.join(
                skechy.app.config["UPLOAD_FOLDER"], file["filename"]
            )
            os.remove(file_path)
...
def create(connection):
    ...
    # Save to disk
    file.save(skechy.app.config["UPLOAD_FOLDER"] / uuid_basename)
    cur = connection.execute(
...
def update_with_picture(connection, logname):
    ...
    # Save to disk
    path = skechy.app.config["UPLOAD_FOLDER"] / uuid_basename
    fileobj.save(path)
    connection.execute(
