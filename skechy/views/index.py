...
@skechy.app.route("/comments/", methods=["POST"])
def handle_comment():
...
@skechy.app.route("/likes/", methods=["POST"])
def handle_like():
...
@skechy.app.route("/uploads/<filename>")
def upload_file(filename):
    """Upload file."""
    if 'username' not in flask.session:
        flask.abort(403)

    upload_folder = skechy.app.config["UPLOAD_FOLDER"]
