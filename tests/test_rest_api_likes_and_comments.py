...
def test_likes_delete(client):
    ...
    response = client.get(
        "/api/v1/posts/3/", headers={"Authorization": f"Basic {credentials}"}
    )
...
def test_likes_post(client):
    ...
    # Verify number of likes
    response = client.get(
        "/api/v1/posts/4/", headers={"Authorization": f"Basic {credentials}"}
    )
    assert response.status_code == 200
    assert response.get_json()["likes"] == {
...
def test_comments_delete(client):
    ...
    # DELETE comment
    response = client.delete(
        "/api/v1/comments/1/", headers={"Authorization": f"Basic {credentials}"}
    )
    assert response.status_code == 204
