...
def test_posts_list(client):
    ...
    response = client.get(
        "/api/v1/posts/", headers={"Authorization": f"Basic {credentials}"}
    )
    ...
    assert response_json == {
        "next": "",
        "results": [
            {"postid": 3, "url": "/api/v1/posts/3/"},
            {"postid": 2, "url": "/api/v1/posts/2/"},
            {"postid": 1, "url": "/api/v1/posts/1/"},
        ],
        "url": "/api/v1/posts/",
    }
...
def test_posts_detail(client):
    ...
    assert response_json == {
        ...
        "likes": {"lognameLikesThis": True, "numLikes": 1, "url": "/api/v1/likes/6/"},
        "owner": "awdeorio",
...
def test_posts_pagination_simple(client):
    ...
    for _ in range(11):
        connection.execute(
            "INSERT INTO posts(owner, filename) " "VALUES('awdeorio', 'fox.jpg') ",
        )
    connection.commit()
    connection.close()
    ...
    assert response_json == {
        ...
        "results": [
            {"postid": 15, "url": "/api/v1/posts/15/"},
            {"postid": 14, "url": "/api/v1/posts/14/"},
            {"postid": 13, "url": "/api/v1/posts/13/"},
            {"postid": 12, "url": "/api/v1/posts/12/"},
            {"postid": 11, "url": "/api/v1/posts/11/"},
            {"postid": 10, "url": "/api/v1/posts/10/"},
            {"postid": 9, "url": "/api/v1/posts/9/"},
            {"postid": 8, "url": "/api/v1/posts/8/"},
            {"postid": 7, "url": "/api/v1/posts/7/"},
            {"postid": 6, "url": "/api/v1/posts/6/"},
        ],
        "url": "/api/v1/posts/",
    }
...
def test_posts_pagination_page_size(client):
    ...
    # GET page 1 size 6
    response = client.get(
        "/api/v1/posts/?size=6", headers={"Authorization": f"Basic {credentials}"}
    )
    assert response.status_code == 200
    response_json = response.get_json()
...
def test_posts_pagination_errors(client):
    ...
    response = client.get(
        "/api/v1/posts/1000/", headers={"Authorization": f"Basic {credentials}"}
    )
    ...
    response = client.get(
        "/api/v1/posts/1000/likes/", headers={"Authorization": f"Basic {credentials}"}
    )
    assert response.status_code == 404

    response = client.get(
        "/api/v1/posts/?page=-1", headers={"Authorization": f"Basic {credentials}"}
    )
    assert response.status_code == 400

    response = client.get(
        "/api/v1/posts/?size=-1", headers={"Authorization": f"Basic {credentials}"}
    )
    assert response.status_code == 400
